...
parser.add_argument('--root_path', type=str, default='../data/2022LA_Seg_Training Set/', help='Name of Experiment')
parser.add_argument('--model', type=str,  default='yu', help='model_name')
parser.add_argument('--method', type=str,  default='result_fedlab04', help='model_name')
...
parser.add_argument('--model_idx', type=int,  default=99, help='GPU to use')
FLAGS = parser.parse_args()

os.environ['CUDA_VISIBLE_DEVICES'] = FLAGS.gpu
model_path = "../fed_output(DP)/"
snapshot_path = "../fed_output(DP)/"+FLAGS.method+"/"
...
for client_idx in range(client_num):
    client_data_list.append(glob('../dataset/miniData/Fundus/{}/*.npy'.format(client_name[client_idx])))
    # client_data_list.append(glob('./dataset/{}/data_npy/*.npy'.format(client_name[client_idx])))
...
def test(site_index, test_net_idx):
   ...
   #  save_mode_path = "../gpu_output/yu/model/epoch_99.pth"
   #  t1 = torch.load(save_mode_path)
   #  print('t1',type(t1))
    # print('t1',t1.shape)

    save_mode_path = os.path.join(model_path + '/model', "epoch_2022-05-08-12-32-54.pth")
    test_net.load_state_dict(torch.load(save_mode_path))
    test_net.train()

    test_data_list = client_data_list[site_index]
    #test_data_list = testclient_data_list[0]
    dice_array = []
    haus_array = []
...
if __name__ == '__main__':
    ...
    with open(os.path.join(snapshot_path, 'testing_result.txt'), 'a') as f:
        # for test_net_idx in range(10,11):
            ...
            print("dice",(dice[0] + dice[1]) / 2)

            print(("   OD haus is: {}, std is {}, array is {}".format(haus[0], np.std(haus_array[:, 0]), haus_array[:, 0])), file=f)
            print(("      {}".format(haus_array[:, 0])), file=f)
            print(("   OC haus is: {}, std is {}, array is {}".format(haus[1], np.std(haus_array[:, 1]), haus_array[:, 1])), file=f)
            print(("      {}".format(haus_array[:, 1])), file=f)
            print ("haus",(haus[0]+haus[1])/2)
