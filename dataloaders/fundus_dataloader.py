...
class Dataset(Dataset):
    ...
    def __init__(self, client_idx=None, freq_site_idx=None, split='train', transform=None):
        ...
        if split=='train':
            self.image_list = glob.glob('../../dataset/{}/data_npy/*'.format(self.client_name[client_idx]))
            # self.image_list = glob.glob('../dataset/{}/data_npy/*.npy'.format(self.client_name[client_idx]))
            for i in range(len(self.client_name)):
                freq_list = glob.glob('../../dataset/{}/freq_amp_npy/*'.format(self.client_name[i]))
                # freq_list = glob.glob('../dataset/{}/freq_amp_npy/*.npy'.format(self.client_name[i]))
        ...
        #print("in_datdaset_imagelist", self.image_list)
       # print("freq_list_clients",self.freq_list_clients)
    ...
    def __getitem__(self, idx):     #根据索引获取数据
       ...
       # raw_file = "./dataset/client2/data_npy\sample61.npy"
       # print("in_getitem_imagelist", self.image_list)
       # print("shape_in_getitem_imagelist", self.image_list.shape)
       #  print("idx",idx)
       #  print("raw_file",raw_file)
        mask_patches = []

        raw_inp = np.load(raw_file)     #把索引的图片存为numpy形式
        # print("shape_in_getitem_imagelist", raw_inp.shape)
        # print(type(raw_inp))
        image_patch = raw_inp[..., 0:3]
        mask_patch = raw_inp[..., 3:]
        # print("image_patch", image_patch)
        # print("image_patch_shape", image_patch.shape)
       # print("mask_patch_shape",mask_patch.shape)
        image_patches = image_patch.copy()
        #print("img_patches0.shape", image_patches.shape)
        # image_patches = 
        ...
        for tar_freq_domain in np.random.choice(self.freq_site_index, 2):
            ...
            # print("img_patches1.shape", image_patches.shape)
            # print("image_patch_freq_1", image_patch_freq_1.shape)
            image_patches = np.concatenate([image_patches,image_patch_freq_1], axis=-1)

       # print("img_patches2.shape", image_patches.shape)
        image_patches = image_patches.transpose(2, 0, 1)  # 9 * 384 * 384
...
def source_to_target_freq( src_img, amp_trg, L=0.1 ):
    # exchange magnitude    交换幅度
    # input: src_img, trg_img
    src_img = src_img.transpose((2, 0, 1))   # 不知道为啥要转置轴，所以先注释掉，变成 3 *384 * 384 通道数变到第一位
    ...
    # print("amp_src.shape",amp_src.shape)
    # print("pha_src.shape",pha_src.shape)
    # mutate the amplitude part of source with target
    # 用目标改变源的振幅部分

    amp_trg_np = amp_trg.transpose((2, 0, 1))
    # print('amp_trg_np.shape', amp_trg_np.shape)
    amp_src_ = low_freq_mutate_np( amp_src, amp_trg_np, L=L )
