...
class ClientContTrainer(ClientTrainer):
    ...
    def train(self, model_parameters) -> None:
        ...
        writer = SummaryWriter('../../fed_output/event/client_event{}/'.format(self.client_idx))
        for epoch_num in tqdm(range(self.epochs), ncols=70):  # tqdm进度条库
            self._model.train()
            iter_num = 0
            # print("epoch_num",epoch_num)
            for i_batch, sampled_batch in enumerate(self._data_loader):
                # print("i_batch,client_idx",i_batch,self.client_idx)
                # obtain training data  获取训练所需数据
                ...
                # obtain updated parameter at inner loop   元学习
                # 采用元学习(meta-learning)范式，将当前客户端的原始数据集用于meta-train过程，完成outer-loop的更新
                # 将插值后的k-1个新数据集用于meta-test过程，完成在当前客户端的inner-loop的参数更新
                # print("net_current(volume_batch_raw)",net_current(volume_batch_raw)[0].shape)  #len =3
                ...
                if iter_num % 1 == 0:
                    ...
                    self._LOGGER.info(
                        ...
                        (epoch_num, self.client_idx, iter_num, len(self._data_loader), loss_inner.item(),
